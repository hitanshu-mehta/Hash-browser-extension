...
export class VaultEffects {
  loadVault$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(() =>
        this.vaultService.loadVault().pipe(
  ...
  updateVaultItem$ = createEffect(() =>
    this.actions$.pipe(
      ...
      pluck('vaultItem'),
      mergeMap((vaultItem) =>
        concat(of(VaultActions.removeVaultItem({ id: vaultItem.id })), of(VaultActions.addVaultItem({ vaultItem })))
      )
    )
  );
  ...
  encryptVault$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap((vaultItem: VaultItem) =>
        from(this.vaultService.encryptPassword(vaultItem.password as string)).pipe(
          map((encryptionKeyObj: EncryptionKeyObj) => {
            ...
            return VaultApiActions.encryptVaultSuccess({
              vaultItem: encVaultItem,
            });
          }),
  ...
  addVaultItemAPI$ = createEffect(() =>
    this.actions$.pipe(
      ...
      exhaustMap((vaultItem) =>
        this.vaultService.addVaultItem(vaultItem).pipe(
  ...
  removeVaultItemAPI$ = createEffect(() =>
    this.actions$.pipe(
      ...
      pluck('id'),
      exhaustMap((id) =>
        this.vaultService.removeVaultItem(id).pipe(
  ...
  getVaultItemAPI$ = createEffect(() =>
    this.actions$.pipe(
      ...
      pluck('id'),
      exhaustMap((id) =>
        this.vaultService.getVaultItem(id).pipe(
  ...
  decryptVault$ = createEffect(() =>
    this.actions$.pipe(
      ...
      pluck('vaultItem'),
      exhaustMap((vaultItem: VaultItem) =>
        this.vaultService.decryptPassword(vaultItem.password as EncryptionKeyObj).pipe(
          map((password) => {
            const decVaultItem: VaultItem = Object.assign({}, vaultItem);
            decVaultItem.password = password;
            return VaultApiActions.decryptVaultSuccess({
              vaultItem: decVaultItem,
            });
          }),
  ...
  constructor(private actions$: Actions<VaultActions.VaultActionsUnion>, private vaultService: VaultService) {}
}
