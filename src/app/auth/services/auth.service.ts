...
export class AuthService {
  ...
  async login({ username, password }: Credentials): Promise<User> {
    ...
    if (this.workerAvailable) {
      ...
      isVerified = verifyMasterPassword(this.masterPasswordObj, password);
    }
  ...
  async signup({ username, password }: Credentials): Promise<{
    user: User;
    masterPasswordObj: MasterPasswordObj;
  ...
  async checkMasterPasswordPresent(): Promise<boolean> {
    if (!this.masterPasswordObj) {
      await this.storageService.get<MasterPasswordObj>('MasterPasswordObj').then((obj: MasterPasswordObj) => {
        this.masterPasswordObj = obj;
      });
    }
